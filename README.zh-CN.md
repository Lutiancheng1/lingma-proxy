...
| 本地路径授权与文件 CRUD | 支持。内置 safe_file_* 安全文件工具，默认只开放应用 workspace；额外路径在高级设置中按 read/write/delete 分级授权，聊天口述授权只提供只读访问。 |
...
- **右键检查元素**：本地包、正式 Release 包和 Feishu artifact 桌面包都保留 Wails DevTools，可在应用里右键选择 `Inspect Element` 打开 WebView 控制台排查样式和运行时问题。
...
`./scripts/rebuild-local-app.sh`、正式 Release workflow 和 Feishu artifact workflow 都会使用 Wails `-devtools` 与 `main.devtoolsBuild=true` 构建桌面包，因此安装后的包保留右键 `Inspect Element` 菜单。本地调试包只有在明确执行 `ENABLE_DEVTOOLS=0 ./scripts/rebuild-local-app.sh` 时才关闭该菜单。
