...
- 把飞书 / Lark Bot 消息接入 Lingma Proxy，让飞书聊天可以直接使用本机 Lingma 后端模型。
- Bot 可以按本机凭据和权限调用基于 `lark-cli` 的飞书工具，覆盖消息、文档、日历、知识库等飞书工作区操作。
- 支持 Feishu CardKit 风格的流式回复；长回答可以逐步更新到飞书消息里，不必等整段生成完成。
- 本地文件访问按权限收口：默认开放应用 workspace，额外读 / 写 / 删除路径需要在 Feishu Agent 高级设置中显式配置。
- 定时提醒类任务到点后可以直接通过飞书渠道发送提醒；需要模型继续处理的 agent 类定时任务仍可走模型执行。
- Feishu Agent 侧管理会话上下文、工具结果记忆、导入的 Skills，以及可选 MCP 工具，便于支撑更长的工作会话。
