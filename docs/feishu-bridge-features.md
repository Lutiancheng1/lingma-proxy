...
Agent 内置的工具不是简单的 API 封装。每个工具都有结构化的参数定义、错误处理和结果分类策略。
...
Bridge 内置完整的 MCP（Model Context Protocol 2025-06-18）客户端。这意味着你可以把任何工具通过 MCP 接入飞书 Agent。
...
Agent 不能自己执行 `auth login`。遇到权限不足时，Bridge 自动发起设备流登录，返回授权链接给用户，用户授权后才继续。LLM 始终没有直接操作凭证的能力。
...
2. **权限错误**：Bridge 自动发起设备流登录，返回授权链接
...
文件访问白名单 + 确认覆盖/删除保护 + 路径逃逸防护 + 授权流程由 Bridge 控制。LLM 不能直接执行 `auth login`，不能绕过权限白名单。
...
| | ChatGPT 飞书集成 | Claude 飞书集成 | Lingma Feishu Agent |
|---|---|---|---|
| 飞书 API 深度集成 | 无 | 无 | 30+ 结构化工具 |
| 流式卡片输出 | 纯文本 | 纯文本 | CardKit 打字机效果 |
| 上下文管理 | 简单截断 | 简单截断 | 四级水位线 + 智能分类 |
| 工具记忆 | 无 | 无 | FTS5 全文搜索 RAG |
| 飞书历史搜索 | 无 | 无 | 主动检测 + 搜索注入 |
| MCP 扩展 | 无 | 无 | 完整客户端 |
| 定时任务 | 无 | 无 | cron + 一次性延迟 |
| 文件安全 | N/A | N/A | 白名单 + 确认删除 |
| 本地部署 | 云端 | 云端 | 本地桌面应用 |
| 开源 | 否 | 否 | 是 |

ChatGPT 和 Claude 的飞书集成，本质上就是把飞书当成了一个消息通道。你在飞书里发消息，它转给 ChatGPT/Claude 处理，然后把回复贴回来。它不能帮你查日历、搜文档、改权限、建任务。

Lingma Feishu Agent 不一样。它跑在你本机，有完整的飞书 API 操作能力，有记忆，有上下文管理，有错误恢复，有定时任务。它不是一个"飞书里的 ChatGPT"，它是一个真正的飞书 AI 员工。
